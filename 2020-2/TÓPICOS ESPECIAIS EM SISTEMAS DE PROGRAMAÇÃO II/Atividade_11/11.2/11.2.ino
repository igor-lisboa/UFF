...
/* Access Token */
#define ACCESS_TOKEN "SJBQFRjDDXPnkqfG8Dyi"
...
const char* BROKER_MQTT = "demo.thingsboard.io"; 
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    Serial.println(msg);
    //verifica mensagens enviadas (ON/OFF)
    if (msg == "{\"method\":\"setValue\",\"params\":true}") {
      digitalWrite(LED, HIGH);
      Serial.println("LED ligado");
    } else if (msg == "{\"method\":\"setValue\",\"params\":false}") {
...
void reconnect_mqtt(void) 
{
    while (!MQTT.connected()) 
    {
        ...
        if (MQTT.connect(ID_MQTT, ACCESS_TOKEN, "")) 
        {
